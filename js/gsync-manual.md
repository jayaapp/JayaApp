...
4. **Configure Client ID and App-Specific Settings**
...
5. **⚠️ CRITICAL: Configure App-Specific localStorage Keys**
   
   When reusing this module in a different app, you **MUST** update the localStorage key prefixes in `gsync-ui.js` to match your app name. This ensures deletion events are properly tracked.
   
   **Find and replace in `gsync-ui.js`:**
   
   ```javascript
   // Line ~387 - In addDeletionEvent() method
   localStorage.getItem('jayaapp-pending-deletions')  // Change 'jayaapp' to 'yourapp'
   localStorage.setItem('jayaapp-pending-deletions')  // Change 'jayaapp' to 'yourapp'
   
   // Line ~401 - In getPendingDeletionEvents() method  
   localStorage.getItem('jayaapp-pending-deletions')  // Change 'jayaapp' to 'yourapp'
   localStorage.removeItem('jayaapp-pending-deletions')  // Change 'jayaapp' to 'yourapp'
   
   // Line ~243 - In getDeviceId() method (optional but recommended)
   localStorage.getItem('jayaapp-device-id')  // Change 'jayaapp' to 'yourapp'
   localStorage.setItem('jayaapp-device-id')  // Change 'jayaapp' to 'yourapp'
   ```
   
   **Example for an app named "MyBookReader":**
   ```javascript
   // Change from:
   localStorage.getItem('jayaapp-pending-deletions')
   // To:
   localStorage.getItem('mybookreader-pending-deletions')
   ```
   
   **Why this matters:** The deletion tracking system uses localStorage to store pending deletion events. If the key names don't match between the methods that write and read deletion events, deletions won't sync across devices, causing deleted items to reappear.
...
When integrating gsync modules into a new application, follow this checklist:

### Required Configuration Steps

- [ ] **1. Update Client ID** in `gsync-integration.js`
  ```javascript
  syncManager.configure('YOUR-CLIENT-ID.apps.googleusercontent.com');
  ```

- [ ] **2. Update Sync Filename** in `gsync-integration.js`
  ```javascript
  const syncManager = new GoogleDriveSync({
      fileName: 'your-app-name-sync.json',  // Change this
      onStatusChange: (status) => { ... }
  });
  ```

- [ ] **3. Update localStorage Keys** in `gsync-ui.js` (search and replace):
  - `jayaapp-pending-deletions` → `yourapp-pending-deletions` (2 locations)
  - `jayaapp-device-id` → `yourapp-device-id` (2 locations)

- [ ] **4. Update Data Collection** in `gsync-ui.js` `performCompleteSync()` method (~line 106):
  ```javascript
  const localData = {
      bookmarks: JSON.parse(localStorage.getItem('bookmarks') || '{}'),
      notes: JSON.parse(localStorage.getItem('your-notes-key') || '{}'),
      // Add your app's data structures here
  };
  ```

- [ ] **5. Update localStorage Keys** in `gsync-ui.js` `updateLocalStorage()` method (~line 328):
  ```javascript
  localStorage.setItem('bookmarks', JSON.stringify(mergedData.bookmarks));
  localStorage.setItem('your-notes-key', JSON.stringify(mergedData.notes));
  // Update to match your app's localStorage keys
  ```

- [ ] **6. Add Deletion Tracking** to your app's delete operations:
  ```javascript
  // Whenever you delete an item, add:
  if (window.syncUI) {
      window.syncUI.addDeletionEvent(itemKey, 'itemType');
  }
  ```

- [ ] **7. Test Cross-Device Sync**:
  - Create items on Device A
  - Sync and verify they appear on Device B
  - Delete items on Device B
  - Sync and verify deletions propagate to Device A

### Common Integration Mistakes

❌ **Mistake 1:** Forgetting to update localStorage key prefixes in `gsync-ui.js`
- **Result:** Deletion events never sync, deleted items reappear
- **Fix:** Search for `jayaapp-` and replace with your app prefix

❌ **Mistake 2:** Not calling `addDeletionEvent()` when deleting items
- **Result:** Deletions are local-only, items resurrect from other devices
- **Fix:** Add `window.syncUI.addDeletionEvent(key, type)` to all delete operations

❌ **Mistake 3:** localStorage key mismatch between data collection and storage
- **Result:** Data doesn't sync properly
- **Fix:** Ensure keys match your app's actual localStorage structure

❌ **Mistake 4:** Loading gsync scripts in wrong order
- **Result:** Initialization failures, "undefined" errors
- **Fix:** Load in order: gsync-minimal.js → gsync-ui.js → gsync-integration.js
